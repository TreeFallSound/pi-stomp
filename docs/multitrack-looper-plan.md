...
| Longpress → secondary CC (was plan item 3) | **Deferred** | The `longpress` config enum in `pistomp/config.py` needs to grow arbitrary "send this CC" actions, the same way short-press bindings already do. Until then, the looper's longpress-as-clear binding can be exercised in mod-ui by MIDI-learning a second footswitch CC to the `reset` port. |
