...
class Parameter:
    def __init__(
        ...
        # Writes go through reconcile/preview/commit, never a raw setter.
        # _confirmed is the value a failed commit rolls back to.
    ...
    def reconcile(self, value: float) -> None:
        """Adopt the single writer's value: repaint, confirm, notify committed.
        Notifies even when unchanged, to refresh a keycap a preview left alone."""
        self._confirmed = value
        self._set(value)
        self._notify_committed()

    def preview(self, value: float) -> None:
        """An optimistic local move, not yet confirmed: repaint only."""
        self._set(value)

    def commit(self, value: float, sink: ParamSink | None) -> bool:
        """A finished local edit: repaint, send through *sink*, confirm, notify.
        Rolls back to the confirmed value and returns False if the send fails.
        A good commit confirms itself — the no-echo paths get no reconcile."""
        self._set(value)
    ...
    def set_binding_range(self, binding_range: tuple[float, float]) -> None:
        """Set the effective extents from a MIDI-CC (sub-)range and reclamp."""
        if (self.minimum, self.maximum) != binding_range:
            self.minimum, self.maximum = binding_range
            self._reclamp()

    def clear_binding_range(self) -> None:
        """Restore effective extents to the plugin's declared LV2 range and reclamp."""
    ...
    def _reclamp(self) -> None:
        """The extents moved: pull value and confirmed inside them, then notify."""
    ...
    def on_commit(self, cb: Callable[[Parameter], None]) -> Callable[[], None]:
        """Register *cb* for confirmed values only (reconcile or committed edit,
        never a bare preview), fired even when unchanged. Returns an unsubscriber."""
