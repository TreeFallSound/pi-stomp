...
class WifiMenu:
    ...
    def open(self, event: object = None, widget: object = None) -> None:
        ...
        supported = bool(util.DICT_GET(wifi_status, 'wifi_supported'))
    ...
    def _build_items(self, rows: list[Row], extras: list[Row],
        ...
        items: list[MenuItem] = [(self._row_label(r), self._on_network_tap, r) for r in rows]
        if extras:
            items.append(("More saved...", self._open_more_saved, extras))
        items.append(("Join other network...", self._open_join_dialog, None))
        items.append((
            "Hotspot Mode  " + (HOTSPOT_ON if hotspot_active else HOTSPOT_OFF),
    ...
    def _on_network_tap(self, row: Row) -> None:
        if row.get('saved'):
            self._open_saved_submenu(row)
        elif is_open_network(row.get('security')):
    ...
    def _open_saved_submenu(self, row: Row) -> None:
        items: list[MenuItem] = []
    ...
    def _open_more_saved(self, extras: list[Row]) -> None:
        items: list[MenuItem] = [(self._row_label(r), self._on_network_tap, r) for r in extras]
    ...
    def _connect_saved(self, row: Row) -> None:
        ...
        self._connect_with_feedback(
            lambda: self._wifi_manager.connect_saved(profile['name']),
            row['ssid'])
    ...
    def _connect_with_feedback(self, connect_fn: ConnectFn, ssid: str) -> None:
        ...
        self.lcd.draw_info_message("", refresh=True)
        if err is None:
            self.lcd.draw_info_message("connected: %s" % ssid, refresh=True)
            return
