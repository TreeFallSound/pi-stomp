...
def test_progress_border_snapshots_at_integer_beat_syncs(v3_system: SystemFixture, make_parameter, snapshot):
    ...
    for beat_in_bar in range(4):
        ...
        v3_system.ws_bridge.inject(f"beat_sync {source_us} 120.0 4 {beat_in_bar}.0 1")
        handler.poll_ws_messages()
...
def test_progress_border_is_steady_without_transport(v3_system: SystemFixture, make_parameter):
    """With no transport and no tempo there is no beat. The border shows the
    full ring at its steady brightness."""
    handler = v3_system.handler
    plugins = [_loopjefe(make_parameter, "loopjefe_1")]
    _bind(v3_system, plugins)
    ...
    assert progress.mode is LoopFill.FREE and progress.segments == 4


class TestFreeRunningGrid:
    """With no transport, beat_grid runs free from the tap. The beat reaches
    every renderer, but a free grid has no bar. The border thus shows a full
    ring, and it keeps the beat with the tap LED."""

    def _free_run(self, v3_system: SystemFixture, make_parameter):
        """A 4-bar loop in playback, 120 bpm from the tap, no transport."""
        handler = v3_system.handler
        _bind(v3_system, [_loopjefe(make_parameter, "loopjefe_1")])
        v3_system.ws_bridge.inject("output_set /graph/loopjefe_1 state 4.0")
        v3_system.ws_bridge.inject("output_set /graph/loopjefe_1 loop_bars 4.0")
        handler.poll_ws_messages()
        handler.beat_grid.clear()
        tap = v3_system.hw.taptempo
        assert tap is not None
        tap.set_bpm(120.0)  # 500ms period, 50ms flash window
        tap.anchor = 1000.0
        return handler

    def test_border_is_a_full_ring_that_keeps_the_beat(self, v3_system: SystemFixture, make_parameter):
        handler = self._free_run(v3_system, make_parameter)
        fs = v3_system.hw.footswitches[0]

        with patch("modalapi.modhandler._now_us", return_value=int(1000.025 * 1_000_000)):
            handler._drive_footswitch_leds()
            on = handler.footswitch_loop_progress(fs)
        with patch("modalapi.modhandler._now_us", return_value=int(1000.3 * 1_000_000)):
            handler._drive_footswitch_leds()
            off = handler.footswitch_loop_progress(fs)

        assert on is not None and off is not None
        assert on.mode is LoopFill.FREE and off.mode is LoopFill.FREE
        assert on.segments == 4 and on.position == 0.0
        assert (on.pulse, off.pulse) == (1.0, 0.0)

    def test_loop_led_blanks_between_free_beats(self, v3_system: SystemFixture, make_parameter):
        """The METRONOME gate follows the grid, not the transport. Before this,
        a free grid left the loop LEDs solid while the tap LED blinked."""
        handler = self._free_run(v3_system, make_parameter)
        fs = v3_system.hw.footswitches[0]
        fs.pixel = MagicMock()
    ...
    def test_free_beats_never_accent(self, v3_system: SystemFixture, make_parameter):
        """A tap gives a beat, not a bar. No renderer may show a downbeat."""
        handler = self._free_run(v3_system, make_parameter)
        with patch("modalapi.modhandler._now_us", return_value=int(1000.025 * 1_000_000)):
            handler._drive_footswitch_leds()
        beat = handler._last_beat
        assert beat is not None
        assert beat.is_running and not beat.is_anchored
        assert beat.is_flashing and not beat.is_bar_start
