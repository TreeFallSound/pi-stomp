...
def test_toggled_param_collapses_to_on_when_either_side_is_on():
    lower = BlendStop(0.0, 0, {"Fx": {Symbol("Switch"): 0.0}})
    upper = BlendStop(1.0, 1, {"Fx": {Symbol("Switch"): 1.0}})
    getter = _type_getter({("Fx", "Switch"): ParameterType.TOGGLED})

    result = build_segment_diff_map(lower, upper, getter)
    pd = result["Fx"][Symbol("Switch")]
...
def test_default_continuous_param_keeps_both_endpoints():
    lower = BlendStop(0.0, 0, {"Fx": {Symbol("Vol"): 0.2}})
    upper = BlendStop(1.0, 1, {"Fx": {Symbol("Vol"): 0.8}})

    result = build_segment_diff_map(lower, upper, _type_getter({}))
    pd = result["Fx"][Symbol("Vol")]
...
def test_midi_bound_param_is_excluded():
    lower = BlendStop(0.0, 0, {"Fx": {Symbol("Vol"): 0.0, Symbol("Bound"): 0.0}})
    upper = BlendStop(1.0, 1, {"Fx": {Symbol("Vol"): 1.0, Symbol("Bound"): 1.0}})
    result = build_segment_diff_map(lower, upper, _type_getter({}), {("Fx", Symbol("Bound"))})
    assert "Vol" in result.get("Fx", {})
    assert "Bound" not in result.get("Fx", {})


def test_instance_dropped_when_all_its_params_are_excluded():
    lower = BlendStop(0.0, 0, {"Fx": {Symbol("Bound"): 0.0}})
    upper = BlendStop(1.0, 1, {"Fx": {Symbol("Bound"): 1.0}})
    result = build_segment_diff_map(lower, upper, _type_getter({}), {("Fx", Symbol("Bound"))})
    assert "Fx" not in result


def test_equal_value_params_are_dropped():
    lower = BlendStop(0.0, 0, {"Fx": {Symbol("Same"): 0.5}})
    upper = BlendStop(1.0, 1, {"Fx": {Symbol("Same"): 0.5}})
    result = build_segment_diff_map(lower, upper, _type_getter({}))
    assert result == {}
